...
func (s *Server) accounts(w http.ResponseWriter, r *http.Request) {
	...
	for rows.Next() {
		...
		if rows.Scan(&id, &name, &email, &region, &enabled, &created, &mode, &proxy, &regions, &sizes, &image, &interval, &batch, &concurrent, &lifetime, &proxyID, &runtimeStatus, &runtimeDetail) != nil {
			continue
		}
