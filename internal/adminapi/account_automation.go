...
func syncAccountAutomationTx(ctx context.Context, tx *sql.Tx, accountID string) error {
	var region, image string
	var sizesRaw []byte
	var interval, batch, concurrent, lifetime int
	if err := tx.QueryRowContext(ctx, `SELECT COALESCE(preferred_region,''),preferred_sizes,COALESCE(preferred_image,''),auto_interval_seconds,auto_batch_size,auto_max_concurrent,server_lifetime_seconds FROM accounts WHERE id=$1`, accountID).Scan(&region, &sizesRaw, &image, &interval, &batch, &concurrent, &lifetime); err != nil {
		return err
	}
	var sizes []string
	_ = json.Unmarshal(sizesRaw, &sizes)
	...
	raw, err := json.Marshal(map[string]any{"name": "account-auto", "region": region, "size": size, "image": image, "lifetime": lifetime * 1000000000, "ssh_user": "root", "inbound_id": 1, "client_count": 1, "email_prefix": "client"})
	if err != nil {
		return err
	}
	var profileID string
