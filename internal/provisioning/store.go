...
func (s SQLStore) Reserve(ctx context.Context, r Run) (Run, bool, error) {
	...
	err = s.DB.QueryRowContext(ctx, `SELECT id::text,account_id::text,droplet_id::text,state,current_step,attempt,created_at,updated_at FROM provision_runs WHERE account_id=$1 AND droplet_id=$2`, r.AccountID, r.DropletID).Scan(&r.ID, &r.AccountID, &r.DropletID, &r.State, &r.CurrentStep, &r.Attempt, &r.CreatedAt, &r.UpdatedAt)
	return r, false, err
}

func (s SQLStore) Update(ctx context.Context, r Run) error {
	_, err := s.DB.ExecContext(ctx, `UPDATE provision_runs SET state=$3,current_step=$4,attempt=$5,updated_at=now() WHERE id=$1 AND account_id=$2`, r.ID, r.AccountID, r.State, r.CurrentStep, r.Attempt)
	return err
}
